...
public final class Main {
    /**
     * Performs the m ai n operation.
     * @param args the a rg s value
    ...
    private static void showGallery(AppBuilder builder, AppContainer app, AuthService auth) {
        ...
        galleryHolder[0] = new GalleryPanel(
                ...
                () -> {
                    ...
                    final NewItineraryDialog dialog = new NewItineraryDialog(galleryFrame, friends);
                    dialog.setVisible(true);
                    if (!dialog.isConfirmed()) {
                        return;
                    }
                    ...
                    final List<User> selectedFriends = dialog.getSelectedFriends();
                    galleryFrame.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                    new Thread(() -> {
                        try {
                            final Trip created = app.createTrip.execute(dest, date,
                                    LocalTime.of(9, 0), LocalTime.of(18, 0),
     ...
     * Opens the profile editor.
      * @param auth the a ut h value
      * @param owner the o wn er value
      * @param app the a pp value
    ...
    private static User openProfile(AppContainer app, AuthService auth, JFrame owner) {
        ...
        final ProfileDialog dialog = new ProfileDialog(owner, profile,
    ...
    private static void startWebPrototype() throws Exception {
        ...
        final Trip demo = app.createTrip.execute(new CreateTripInputData(
                "Toronto",
