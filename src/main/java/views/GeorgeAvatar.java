...
final class GeorgeAvatar {
    ...
    static ImageIcon icon(int width, int height) {
        final BufferedImage source = load();
        final double scale = Math.min(
                (double) width / source.getWidth(),
                (double) height / source.getHeight());
        final int scaledWidth = Math.max(1, (int) Math.round(source.getWidth() * scale));
        final int scaledHeight = Math.max(1, (int) Math.round(source.getHeight() * scale));
    ...
    private static BufferedImage fallback() {
        final BufferedImage image = new BufferedImage(120, 84, BufferedImage.TYPE_INT_ARGB);
