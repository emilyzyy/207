...
import closeai.adapters.viewmodels.CalendarState;
import closeai.adapters.viewmodels.CalendarViewMode;
...
public final class CalendarPanel extends JPanel {
    ...
    private JPanel dayView(CalendarState state) {
        ...
        if (!state.isTripDate(state.getFocusDate())) {
            timeline.add(emptyLabel("No CloseAI trip is scheduled for this date."));
